...
(https://www.klipper3d.org/) in your Qidi Tech X-Plus or Qidi Tech X-Max
...
-  connect the Raspberry Pi to the serial port on the Qidi X-Plus/X-Max
   Mainboard (I used an USB to Serial Adapter:
   rx->tx/tx->rx/ground->ground; the raspberries own contacts should
   also work)
-  *where to start?*
-  I for myself started with **prind** (docker compose based setup) on
   an Raspberry Pi OS and altered it to my wishes
   but there are many additional hurdles if you're not used to docker so
   I would not recommend this to everybody...
   https://github.com/mkuf/prind
-  Never tried it myself, but would guess that **KIAUH** is probably the
   best option.
   ::

       it's often noted in posts all over the  web: *KIAUH - Klipper Installation And Update Helper*
       https://github.com/th33xitus/kiauh 

-  *how to I configure klipper etc.?*
-  maybe use Funkton's unfortunately outdated config files and start
   from there, I do not have ready ones yet
   ::

       https://discord.com/channels/686160672211730511/821928891491942451/903867198441918487  

-  **!!warning!!** after you can connect klipper to your klippered
   firmware, be sure to do nothing else, but as a first thing follow
   https://www.klipper3d.org/Config\_checks.html
-  **!!warning!! for X-Plus** as Funktons printer.cfg targets an X-Max,
   some values are off
   ::

Necessary changes to print.cfg:
...
[stepper_x]
...
rotation_distance: 40 # for 20T metallic pulleys; go with 32 for the original 16T pulleys and with 34 for 17T pullys (Sidenote: Funktons value fpr 17T: 33.984)
....
...
```
   -  below [stepper\_x] position\_endstop and position\_max should each
      be set to 270
   -  below [stepper\_y] position\_endstop and position\_max should each
      be set to 200
   -  below [stepper\_z] position\_endstop and position\_max should each
      be set to 200, maybe 205
      (I had to set it a little higher, position\_max to 205 to be able to z-level which got position\_endstop to 204.396)

-  *when am I done with chapter 0?*
   you are done with your klipper/moonraker/fluidd stack, if fluidd
   shows one of the following two (but no other errors)
   ::
...
-  Buy ST-Link V2, e.g. https://de.aliexpress.com/item/32792513237.html
-  Update ST-LinkV2 Firmware (is later on forced by STM32CubeProg)
   ...
   (you have to register with email and get an download link in your
   inbox once)
...
-  Unscrew and remove the lower cover so you have access to the
   mainboard

2. Connect the ST-Link to the printer
-------------------------------------

#. **Dual check you have shut down the printer (physical power off
   button in the back)**
#. connect the pins, they are not in the same order but very good
   "matchable"
   (see
   https://discord.com/channels/686160672211730511/821928891491942451/838586898023841872)
   ::
...
#. Settings in STM32CubeProg (see
   https://discord.com/channels/686160672211730511/821928891491942451/838635980272435221)
...
-  case 1: you have the original Bootloader on the mainboard: it will
...
#. click Read in the upper right and then next to that the down arrow
   and save as e.g. "InitialBackup.bin" and store multiple copies
   everywhere :p
...
-  FYI according to
   ...
   flash size should be 512K (I have read "STM32F407ZE" on my X-Plus
   mainboards main chip)

4. (optional) Restore complete original firmware (including bootloader)
-----------------------------------------------------------------------

#. execute chapter 2
#. switch to "Download" (see
   https://discord.com/channels/686160672211730511/821928891491942451/838637467258060820)
#. address should be at 0x08000000
...
#. sidenotes:
#. I used the option USART1 as I connected an USB-to-serial-converter
   Raspberry PI 3b+ to the usb of the raspberry and the serial side to
   the mainboard.
...
-  looked through the Device Memory Bytes...
   ::
       ...
       there is a jump to FFF...FFF at 0x08074000  maybe Qidi firmware is 344.064 bytes / 336k ???
           matches the last isntalled firmware update which was actually 336k in size
...
-  script to convert a compiled klipper.bin to an update.qd, which the
   ...
   usb stick:
   https://github.com/Klipper3d/klipper/blob/master/scripts/update\_chitu.py.
